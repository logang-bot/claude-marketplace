...
description: Limits function and method bodies to ~7 lines and 3 parameters (extras grouped into a data class, record, or struct), requires intent-revealing names, bans explanatory inline comments in favour of doc comments on non-obvious math only, and places each method below the one that calls it. Use when writing or refactoring a function or method, adding a parameter, commenting inside a body, or deciding where a new method goes in a class. UI component functions such as Compose composables, React components, and SwiftUI views are exempt from the size limits. Applies however the code was written — including a function added by a shell command, a generator script, or another tool rather than the file editor, and one written while carrying out an already-approved plan.
---
...
7. **Place a method below the one that calls it**: Within a visibility block, a helper goes directly under its caller, depth first — each helper's own helpers before the caller's next one. If `submit()` calls `a()` then `b()`, and `a()` calls `a1()`, the order is `submit, a, a1, b`. Visibility decides the block; this decides the order inside it, so a private helper joins the private block rather than interrupting the public API.
